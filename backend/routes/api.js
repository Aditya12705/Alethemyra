...
const DocumentUpload = () => {
  ...
  useEffect(() => {
    axios.get(`http://localhost:5000/api/application-status/${userId}`)
      .then(res => {
        setApplicationStatus(res.data.status || 'Not Submitted');
      })
      .catch(() => {
        ...
        <form onSubmit={handleSubmit}>
            ...
            <div style={{
              ...
              <div style={{
                ...
                <div style={{ display: 'flex', gap: '32px', flexWrap: 'wrap' }}>
                  ...
                  <div style={{ flex: 1, minWidth: 320 }}>
                    <FileInput label="PLAN APPROVAL Document" name="planApprovalDoc" onChange={e => setPlanApprovalDoc(e.target.files[0])} optional />
                  </div>
                </div>
                {/* Row 2 */}
                <div style={{ display: 'flex', gap: '32px', flexWrap: 'wrap' }}>
                  <div style={{ flex: 1, minWidth: 320 }}>
                    <FileInput label="KHATA CERTIFICATE" name="khataCertificateDoc" onChange={e => setKhataCertificateDoc(e.target.files[0])} optional />
                  </div>
                  <div style={{ flex: 1, minWidth: 320 }}>
                ...
                <div style={{ display: 'flex', gap: '32px', flexWrap: 'wrap' }}>
                  <div style={{ flex: 1, minWidth: 320 }}>
                    <div style={{ marginBottom: 16 }}>
                      <div style={{ fontWeight: 600, color: '#7C6A4E', fontSize: 16, display: 'flex', alignItems: 'center', marginBottom: 6 }}>
                        BWSSB Documents <span style={{ fontSize: '0.95em', color: '#888', fontWeight: 400, marginLeft: 8 }}>(Optional, up to 3)</span>
                      </div>
                      {[0,1,2].map(idx => (
                        <FileInput
                          key={idx}
                          label={`BWSSB Document ${idx + 1}`}
                  ...
                  <div style={{ flex: 1, minWidth: 320 }}>
                    <div style={{ marginBottom: 16 }}>
                      <div style={{ fontWeight: 600, color: '#7C6A4E', fontSize: 16, display: 'flex', alignItems: 'center', marginBottom: 6 }}>
                        KEB Documents <span style={{ fontSize: '0.95em', color: '#888', fontWeight: 400, marginLeft: 8 }}>(Optional, up to 3)</span>
                      </div>
                      {[0,1,2].map(idx => (
                        <FileInput
                          key={idx}
                          label={`KEB Document ${idx + 1}`}
                          name={`keb${idx+1}Doc`}
          ...
          <div style={{
            ...
            <div style={{
              ...
              <div>
                <FileInput label="Ownership Documents" name="ownershipDocuments" onChange={e => setOwnershipDoc(e.target.files[0])} />
              </div>
              <div>
                <FileInput label="Land Document" name="landDoc" onChange={e => setLandDoc(e.target.files[0])} />
              </div>
              <div>
