...
app.get('/api/users', async (req, res) => { // Made async
  try {
    ...
    const results = await db.query(`
      ...
      FROM users
    `);
    console.log(`Successfully fetched ${results.rows.length} users for admin dashboard.`);
    if (results.rows.length > 0) {
      console.log('First user fetched:', results.rows[0]);
    }
    res.json(results.rows); // PostgreSQL results are in .rows
  } catch (err) {
    console.error('Detailed error in /api/users (fetching all users):', {
      message: err.message,
      stack: err.stack,
      code: err.code
    });
    res.status(500).json({ 
      success: false, 
      message: 'Error fetching users for admin dashboard',
...
app.get('/api/user/:id', async (req, res) => { // Made async
  ...
  try {
    ...
    if (results.rows.length > 0) {
      console.log(`Successfully fetched details for user ID: ${id}`);
      console.log('User details:', results.rows[0]);
    ...
    console.error(`Error getting user details for ID ${id}:`, { 
      message: err.message,
      stack: err.stack,
      code: err.code
    });
